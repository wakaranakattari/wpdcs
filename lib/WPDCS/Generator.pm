...
## @param <outdir: str - optional output directory, default alongside source>
...
generate_docs
{
  ...
  ## @info <see values pointing at sources render as doc links>
  my $render_see = sub {
    my ($val) = @_;
    if ($val =~ m{^([\w\-./]+\.(?:pl|pm|clj|cljs|cljc|t))(.*)$})
    {
      my ($path, $rest) = ($1, $2);
      my $md = $path;
      $md =~ s{.*\/}{};
      $md =~ s/\.(pl|pm|clj|cljs|cljc|t)$/.md/;
      print $fh "- **see:** [$path]($md)$rest\n";
    }
    else
    {
      print $fh "- **see:** $val\n";
...
## @funcinfo <write index.md linking generated docs>
## @param <files: arrayref - source paths that were documented>
## @param <outdir: str - directory holding the .md files>
## @return <str - index path>
## @throws <dies on write errors>
## @ex <generate_index(["a.pl"], "docs") => writes docs/index.md>
sub
generate_index
{
  my ($self, $files, $outdir) = @_;
  my $index = "$outdir/index.md";
  open my $fh, '>:encoding(UTF-8)', $index or die "Cannot write $index: $!";
  print $fh "# documentation index\n\n";
  print $fh "generated by wpdcs $WPDCS::Generator::VERSION on " . $self->today() . ".\n\n";
  my $parser = WPDCS::Parser->new();
  for my $file (sort @$files)
  {
    ...
    $md =~ s/\.(pl|pm|clj|cljs|cljc|t)$/.md/;
    my $tags = $parser->parse_file($file);
    my ($info) = map { $_->{value} } grep { $_->{tag} eq 'info' } @$tags;
    $info //= '';
    print $fh "- [$file]($md)";
    print $fh " — $info" if $info ne '';
    print $fh "\n";
  }
  close $fh or die "Cannot close $index: $!";
  say "generated index: $index";
  return $index;
}
...
## @funcinfo <generate .t skeleton from @ex doctest tags>
## @param <file: str - source .pl/.pm path>
## @return <void>
## @throws <dies if source missing>
## @ex <generate_test("src/calc.pl") => creates t/calc.t>
sub
generate_test
{
  my ($self, $file) = @_;
  die "Usage: generate_test(<file>)\n" unless defined $file && -f $file;

  require WPDCS::Doctest;
  my $dt = WPDCS::Doctest->new();
  my $cases = $dt->cases($file);
  die "No \@ex doctests found in $file\n" unless @$cases;

  my $test_file = $file;
  $test_file =~ s{^.*?/(lib|src)/}{t/};
  $test_file =~ s/\.(pl|pm)$/.t/;
  if ($test_file eq $file)
  {
    $test_file = "t/doctest.t";
  }

  my $lib = ($file =~ m{^(lib|src)/}) ? '..' : '.';
  my $content = <<"EOF";
use strict;
use warnings;
use Test::More tests => @{[scalar @$cases]};
use FindBin;
use lib "\$FindBin::Bin/../$lib/lib";
use lib "\$FindBin::Bin/../lib";

\## \@info <generated from $file, edit freely, sync stays manual>
EOF

  ## @info <subs come straight from required file for direct calls>
  ...
  for my $c (@$cases)
  {
    ...
    if ($c->{fail_expected})
    {
      my $label = "doctest $n (line $c->{line}): $expr";
      $content .= "{\n  my \$r = eval { $c->{expr} };\n";
      $content .= "  ok(!\$@ || defined \$r, \"$label\");\n}\n";
    }
    else
    {
      $content .= "is(scalar($c->{expr}), \"$exp\", \"doctest $n (line $c->{line})\");\n";
    }
  }

  my $dir = $test_file;
  $dir =~ s{/[^/]+$}{};
  if ($dir ne '' && $dir ne $test_file && !-d $dir)
  {
    make_path($dir) or die "Cannot create $dir: $!";
  }

  WPDCS::Core->write_file($test_file, $content);
  say "generated test: $test_file (" . scalar(@$cases) . " cases)";
}
